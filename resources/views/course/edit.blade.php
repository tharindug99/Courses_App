...
<form method="post" action="{{ route('course.update',$courses->id) }}" 
      style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
...
<div style="margin-bottom: 15px;">
    ...
    <input type="text" name="courseHead" value="{{$courses->courseHead}}"
    required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;"/>
</div>

<button type="submit" 
style="display: block; width: 100%; padding: 10px; background-color: #2c3e50; 
color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">Edit</button>
</form>
@endsection